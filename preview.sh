...
start_server() {
  ...
  if command -v gnome-terminal &> /dev/null; then
    gnome-terminal -- bash -c "cd $folder && npm run preview; exec bash"
  elif command -v x-terminal-emulator &> /dev/null; then
    x-terminal-emulator -e "bash -c 'cd $folder && npm run preview; exec bash'"
  elif command -v konsole &> /dev/null; then
    konsole -e "bash -c 'cd $folder && npm run preview; exec bash'"
  else
    echo "No supported terminal emulator found. Running in background..."
    (cd "$folder" && npm run preview) &
  fi
}
...
for folder in "${!folders[@]}"; do
  (
    ...
    # Build the project
    echo "Running 'npm run build' in $folder..."
    npm run build
  ) &
done
